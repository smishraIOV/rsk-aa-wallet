...
//    using SafeERC20 for IERC20;
    ...
    // encode it. Can base the method on the one for legacy transaction
    ...
    { 
        ...
        // todo(shree) add the from/sender addr as well 
        bytes memory encodedFrom = RLPEncoder.encodeAddress(address(uint160(_transaction.from)));
        ...
        unchecked {
            ...
            //todo(shree) this triggers compilation error below when optimizer is enabled. need via-ir
        ...
        return
            keccak256(
                bytes.concat(
                    "\x03",  //todo(shree) use the same mask has RSKJ
                    encodedListLength,  
                    encodedNonce,
                    encodedGasParam,
                    encodedTo,
    ...
    // they encourage use of this for future, e.g. other signature schemes
